 ...
 *
 * The 384x56 logical layout is scaled to 2188 samples by 202 scanlines.
 */
enum {
    UI_ROOT = 22,
    UI_HEADER = 24,
...
static inline void menu_ui_pixel(int x, int y, uint8_t code)
{
    ...
    memset(&s_menu_raster.ui[(unsigned)y / 2u][x0], code, x1 - x0);
}
...
static void menu_ui_rect(int x, int y, int w, int h, uint8_t code)
{
    ...
    unsigned sy0 = (unsigned)y0 / 2u;
    unsigned sy1 = ((unsigned)y1 + 1u) / 2u;
    if (sy1 > MENU_UI_STORAGE_LINES) sy1 = MENU_UI_STORAGE_LINES;
    for (unsigned yy = sy0; yy < sy1; ++yy) {
