...
framebuffer. The production UI stores **384 x 48 logical pixels** below the former status strip:

- horizontal coordinates use 207/50 DAC samples (1592 samples / 39.8 us UI width);
- one logical Y row = four physical video lines (192-line UI height);
- exact six-bit DAC shades only; no alpha, anti-aliasing or browser-style scaling;
- persistent navigation rail + page content, rendered from the existing 8x8
  bitmap font and small built-in icons.
...
The compact layout keeps the sidebar and page structure without resampling font rows.

The scatter chain still uses one UI segment per displayed scanline, so the DMA
node count does not grow with glyph complexity. Its capacity is 6,540 nodes to
cover the 4x PAL raster. Only `raster.ui` changes while
