...
- horizontal coordinates scale by 5.7 (2188 samples / 54.7 us UI width);
- vertical coordinates scale by 3.6 (202-line UI height);
...
The 384x56 logical layout is 90% wider and 20% taller than the preceding
1152x168 raster. Vertical scaling reuses logical rows; horizontal scaling is
stored as direct DAC samples so the GDMA loop remains CPU-free.
The vertical backing store uses 28 rows and the renderer maps the 56-coordinate
layout onto them; the raster expands those rows to 202 physical scanlines.
...
cover the PAL raster. Only `raster.ui` changes while
