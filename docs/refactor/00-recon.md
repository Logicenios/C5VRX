...
| T2 | `esp_wifi_start` → PHY init | RF calibration at PHY start. Partial by default (`ESP_PHY_RF_CAL_PARTIAL`, stored data); **full** calibration after the PHY-cal NVS namespace is erased. | **Possible** | The linked PHY contains TX calibration and TX power control (`phy_txcal_work_mode`, `phy_tx_pwctrl_init_cal_new`, `phy_i2c_master_mem_txcap`) [CODE nm]. Espressif does not document whether calibration couples energy to the antenna port. **UNVERIFIED: must be measured at boot with a spectrum analyser / SDR near the antenna.** Phase 2 update: the link map shows `phy_start_tx_tone_step` called from `libphy.a(phy_tx_cal.o)`, `(phy_rx_cal.o)` and `(phy_pwdet.o)`. PHY calibration, including **RX** calibration, generates an internal TX tone. The same symbols are in the upstream 6.0.2 ELF. |
