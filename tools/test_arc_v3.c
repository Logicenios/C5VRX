...
int main(void)
{
    ...
    arc_v3_controller_reset(&a, &t, 62);
    a.settle = 0;
    assert(arc_v3_controller_tick(&a, &obs(65, 99, 450, 0)) == 58);
    a.settle = 0;
    for (int i = 0; i < 3; ++i)
        (void)arc_v3_controller_tick(&a, &obs(36, 100, 0, 0));
    assert(a.gain == 57);
    a.settle = 0;
    for (int i = 0; i < 3; ++i)
        (void)arc_v3_controller_tick(&a, &obs(17, 99, 0, 0));
    assert(a.gain == 57 && a.state == ARC_V3_LOCK);

    /* Close hardware pattern: repeated strong overload keeps moving down. */
    arc_v3_controller_reset(&a, &t, 62);
    a.settle = 0;
    assert(arc_v3_controller_tick(&a, &obs(65, 99, 400, 0)) == 58);
    a.settle = 0;
    assert(arc_v3_controller_tick(&a, &obs(64, 100, 410, 0)) == 54);
    a.settle = 0;
    assert(arc_v3_controller_tick(&a, &obs(65, 99, 480, 0)) == 50);
    ...
    (void)arc_v3_controller_tick(&a, &obs(65, 99, 300, 0));
    assert(a.gain == 77);
